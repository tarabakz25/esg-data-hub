...
ESG（環境・社会・ガバナンス）データの管理と分析を行うWebアプリケーションです。
...
初回セットアップ時に管理者ユーザーを作成してください。管理者は以下の権限を持ちます：
...
管理者でログイン後、以下のAPIエンドポイントを使用してユーザーを追加できます：
...
POST /api/auth/register
{
  ...
  "name": "ユーザー名",
  "password": "パスワード",
  "role": "viewer", // "admin", "ir_manager", "auditor", "viewer"
  "department": "部署名"
}
```
...
- **データ取り込み**: CSVファイルのアップロードと自動解析
- **KPIマッピング**: AI支援による自動的なKPI識別とマッピング
- **コンプライアンスチェック**: ESG規制基準との適合性確認
- **ダッシュボード**: リアルタイムデータの可視化
- **通知システム**: 重要な更新とアラートの配信
...
| **viewer** | データとレポートの閲覧のみ |

## 💾 データベース

PostgreSQLを使用し、以下の主要テーブルで構成されています：

- `standard_kpi_definitions`: 標準KPI定義
- `User`: ユーザー情報
- `Upload`: ファイルアップロード履歴
- `ComplianceCheckResult`: コンプライアンスチェック結果
- `Notification`: 通知システム
...
**初回セットアップを完了したら、管理者アカウントでログインしてシステムの利用を開始してください。**
